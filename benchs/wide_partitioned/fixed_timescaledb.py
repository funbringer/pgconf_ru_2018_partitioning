...
class FixedTimescalePartedTblState:

    #  create_part_i = ""
    insert_part_i = "insert into foo values (now() - interval '{0} hour')"
    select = "select * from foo where t = timestamp '{0}'"
    ...
    def random_select(self):
        random_ts = datetime.datetime.utcnow() - datetime.timedelta(hours=random.randrange(self.partnum))
        return self.select.format(random_ts.strftime('%Y-%m-%d %H:%M:%S'))# % random.randrange(1, self.partnum+1)
